...
internal sealed class DataAccessLayer
{
    ...
    public Database ReadDatabase()
    {
        ...
        {
            Log.Logger.Error(ex, "Error reading database");
    ...
    private void LogTables(List<string> tables)
    {
        ...
        {
            Log.Logger.Debug(table);
            Console.WriteLine(table);
        }
    }

    private static void LogSchema(System.Data.DataTable schema)
    {
        foreach (System.Data.DataRow row in schema.Rows)
        {
            foreach (System.Data.DataColumn column in schema.Columns)
            {
                Log.Logger.Debug($"{column.ColumnName}: {row[column]}");
                Console.WriteLine($"{column.ColumnName}: {row[column]}");
            }
        }
    }

    private static void LogTableSchema(System.Data.DataTable schema)
    {
        foreach (System.Data.DataRow row in schema.Rows)
        {
            Log.Logger.Debug($" {row["ColumnName"]},  {row["DataType"]},  {row["AllowDBNull"]}");
    ...
    private Location ReadLocation(SqliteDataReader reader)
    {
        return new Location
        {
            ...
            // KeySymbol changed to NullableString 2/12/25
            KeySymbol = ReadNullableString(reader, "KeySymbol"),
    ...
    private Note ReadNote(SqliteDataReader reader)
    {
        return new Note
        {
            ...
            Created = ReadString(reader, "Created"),
